...
run_consensus_sequences <- function(
  ...
  # The Docker bash script uses `set +e`, so it can exit 0 even when
  # mkdir/copy operations fail. Verify that at least one consensus FASTA
  # was actually produced to catch silent Docker volume-mount failures.
