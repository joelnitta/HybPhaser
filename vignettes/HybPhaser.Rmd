...
9. Prepare/run BBSplit script for phasing (R)
...
phasing_job <- prepare_phasing_script(
...
cat("\nPhasing script prepared:", phasing_job$script_path, "\n")
```

## Next Steps

After clade-association and phasing script preparation, the typical HybPhaser
workflow continues with:

1. **Phasing result collation** - Summarize phased-read mapping outcomes
2. **Merging** - Combine phased and non-phased sequences
...
- Function documentation: `?count_snps`, `?assess_dataset`,
  ...
  `?prepare_phasing_script`, `?prepare_phasing_script_from_config`
