---
title: "HybPhaser Workflow with targets"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{HybPhaser Workflow with targets}
...
run_targets_vignette <- identical(
  Sys.getenv("HYBPHASER_RUN_TARGETS_VIGNETTE"),
  "true"
)
```
...
{targets}. The analysis steps are the same as the standard workflow vignette,
but dependencies are declared explicitly in a pipeline.
...
This workflow is self-contained. It downloads and runs the official HybPiper
test dataset, then generates clade-association and phasing CSV files inside
the pipeline.

## Create Pipeline Script from the Vignette

`targets::tar_script()` writes a `_targets.R` file from R code. This lets you
define and maintain your pipeline directly in the vignette.

```{r, eval=run_targets_vignette}
targets::tar_dir({
  targets::tar_script(
    {
      library(targets)
...
- To run it end-to-end while rendering, set environment variable
  `HYBPHASER_RUN_TARGETS_VIGNETTE=true` before knitting.
- The pipeline auto-generates all required input files and does not depend on
  pre-existing files in `data/`.
- `run_consensus_sequences()` is used instead of
  `run_generate_consensus_sequences()` because returning a path is more natural
  for `{targets}` dependency tracking.
- `run_phased_hybpiper()` and `make_phased_namelist()` are exported package
  functions, so no custom helper file is required.
- `write_clade_reference_csv()` and `write_phasing_prep_csv()` are exported
  helper functions used to generate CSV inputs inside the pipeline.
- Docker and conda availability are still required when you actually execute
  the pipeline.
